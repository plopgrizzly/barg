...
#[inline(always)]
pub(crate) fn bitmap<'a, I>(
    image: *mut u8,
    wh: Size,
    pitch: usize,
    path: I,
    texture: (u16, u16, &[[u8; 4]]),
    lines: &mut Vec<Line>,
    curves: &mut Vec<Curve>,
    w: f32,
    h: f32,
) where
    I: IntoIterator<Item = &'a PathOp>,
{
    let mut last;
    ...
    loop {
        ...
        rasterizer::rasterize(
            ...
            |x, y, v, xf, yf| {
                ...
                over(src, unsafe {
                    std::slice::from_raw_parts_mut(image.offset(out_index), 4)
                });
            },
        );
    ...
    I: IntoIterator<Item = &'a PathOp>,
{
    ...
    for path_op in path.into_iter() {
        match path_op {
            ...
            PathOp::Cubic(_c1x, _c1y, _c2x, _c2y, _x, _y) => unimplemented!(),
            PathOp::PenWidth(_) => { /*ignore*/ }
            PathOp::Close() => { /*ignore*/ }
        }
    }
    ...
    rasterizer::rasterize(
        ...
        |x, y, v, _xf, _yf| {
            ...
            let src = [color[0], color[1], color[2], util::scale_u8(color[3], v)];
